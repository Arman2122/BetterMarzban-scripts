...
LAST_XRAY_CORES=10
SOURCE_REPO="Arman2122/BetterMarzban"
SCRIPTS_REPO="Arman2122/BetterMarzban-scripts"
DEFAULT_BRANCH="main"
DEV_BRANCH="dev"
MARZBAN_IMAGE_REPO="ghcr.io/arman2122/bettermarzban"
FILES_URL_PREFIX="https://raw.githubusercontent.com/${SOURCE_REPO}/${DEFAULT_BRANCH}"
MARZBAN_IMAGE="${MARZBAN_IMAGE_REPO}:latest"
...
get_existing_github_token() {
    if [ -n "$MARZBAN_GITHUB_TOKEN" ]; then
        echo "$MARZBAN_GITHUB_TOKEN"
...
ensure_github_token() {
    ...
    local token
    token=$(get_existing_github_token)
    
    if [ -z "$token" ]; then
        colorized_echo yellow "Repository access requires a GitHub token with repo scope."
        read -s -p "GitHub token: " token
        echo
    else
        colorized_echo yellow "Using existing GitHub token for repository access."
    fi
    
    if [ -z "$token" ]; then
        colorized_echo red "GitHub token is required to continue."
        exit 1
    fi
    
    GITHUB_AUTH_HEADER=(-H "Authorization: token $token")
    USE_GITHUB_AUTH=1
    export MARZBAN_GITHUB_TOKEN="$token"
    GITHUB_TOKEN="$token"
}

curl_with_repo_auth() {
    if [ $# -lt 1 ]; then
        colorized_echo red "curl_with_repo_auth requires at least the URL argument."
        exit 1
    fi
    ...
    if [ "$USE_GITHUB_AUTH" -eq 1 ]; then
        curl -fsSL "${args[@]}" "${GITHUB_AUTH_HEADER[@]}" "$url"
        return
    fi
    
    if curl -fsSL "${args[@]}" "$url"; then
        return 0
    fi
...
install_marzban_script() {
    SCRIPT_URL="https://github.com/$SCRIPTS_REPO/raw/main/marzban.sh"
    colorized_echo blue "Installing marzban script"
    local temp_file
    temp_file=$(mktemp)
    curl_with_repo_auth -o "$temp_file" "$SCRIPT_URL"
    install -m 755 "$temp_file" /usr/local/bin/marzban
    rm -f "$temp_file"
...
set_release_context() {
    ...
    FILES_URL_PREFIX="https://raw.githubusercontent.com/${SOURCE_REPO}/${ref}"
    MARZBAN_IMAGE="${MARZBAN_IMAGE_REPO}:${image_tag}"
}
...
EOF
        ...
        curl_with_repo_auth -o "$APP_DIR/.env" "$FILES_URL_PREFIX/.env.example"
...
EOF
        ...
        curl_with_repo_auth -o "$docker_file_path" "$FILES_URL_PREFIX/docker-compose.yml"
    ...
    curl_with_repo_auth -o "$DATA_DIR/xray_config.json" "$FILES_URL_PREFIX/xray_config.json"
...
install_command() {
    ...
    check_version_exists() {
        local version=$1
        repo_url="https://api.github.com/repos/${SOURCE_REPO}/releases"
        if [ "$version" == "latest" ] || [ "$version" == "dev" ]; then
            return 0
        fi
        
        # Fetch the release data from GitHub API
        response=$(curl_with_repo_auth "$repo_url")
    ...
    if [[ "$marzban_version" == "latest" || "$marzban_version" == "dev" || "$marzban_version" =~ ^v[0-9]+\.[0-9]+\.[0-9]+$ ]]; then
        if check_version_exists "$marzban_version"; then
            set_release_context "$marzban_version"
